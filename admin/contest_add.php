...
<script src="../include/xx_readUser.js" type="text/javascript"></script>
<script src="../include/xx_gradeReturn.js" type="text/javascript"></script>
<?php
...
?><br>
		年级：
		<select id="s_grade" 
		onchange="selectGrade(this.options[this.options.selectedIndex].value);" >
			<option value="-1" selected= "selected">All</option>
			<?php 
			$sqlStr="select `grade`from `users` group by `grade`";
			$result=mysql_query($sqlStr);
			if($result) {
				while($row = mysql_fetch_array($result)){?>
					<option value="<?php echo iconv("utf-8","utf-8",$row['grade']); ?>" >
						<?php echo iconv("utf-8","utf-8",$row['grade']); ?></option>
					<?php
					}
				}
			?> 
		...
		<select id="s_class">
			<option value="-1" selected= "selected">All</option>
			<?php 
			$sqlStr="select `class` from `users` group by `class`";
			$result=mysql_query($sqlStr);
			if($result) {
				while($row = mysql_fetch_array($result)){?>
					<option value="<?php echo iconv("utf-8","utf-8",$row['class']); ?>" >
						<?php echo iconv("utf-8","utf-8",$row['class']); ?></option>
					<?php
					}
				}
			?> 
		</select>
		<input type="button" id="button" onclick="AjaxReauest()" value="add Users">
		<br />
