...
if (isset($_GET['problem_id'])&&$_GET['problem_id']!=""){
	//echo "problem id is :".$_GET['problem_id']."<br>";
	if(isset($_GET['cid'])){
		$problem_id=$_GET['problem_id'];
		if($problem_rows>=26)//这里同样做判断
		$num=$problem_id;
		else $num=strpos($PID,$problem_id);
		$sql=$sql."AND `num`='".$num."' ";
...
if($OJ_MEMCACHE)
       ...
       if ($row['contest_id']>0) {
                ...
                if(isset($cid)){
                		...
                		if($problem_rows>=26)
                        	$view_status[$i][2].= $row['num'];//even.2013.01.17:相应的题目顺序.
                		else $view_status[$i][2].=$PID[$row['num']];
                }else{
