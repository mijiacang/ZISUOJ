...
if (isset($_GET['cid'])){
		...
		AND `contest_problem`.`contest_id`=$cid ORDER BY `contest_problem`.`num` 
			...
			$num=intval(mysql_num_rows($result));
			//echo "the problem rows is ".$num."<br>";
			$cnt=0;
			while ($row=mysql_fetch_object($result)){
				$view_problemset[$cnt][0]="";
				if (isset($_SESSION['user_id'])) 
					$view_problemset[$cnt][0]=check_ac($cid,$cnt);
				if($num<=26)//even2013.01.14：题目列表的更改,当题目数小于26题时采用英文大写字符的方式，当大于26个时则采用阿拉伯数字计数
					$view_problemset[$cnt][1]= "$row->pid Problem &nbsp;".(chr($cnt+ord('A')));
				else $view_problemset[$cnt][1]= "$row->pid Problem &nbsp;".$cnt;
